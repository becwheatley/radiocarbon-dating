---
title: "Sampling bias in radiocarbon dating project: simulation study (no population change) - mimicking AustArch data"
...
no_sites   = 1500            ## Holocene AustArch has 1,529 site
...
We want to emulate the structure of the AustArch Holocene radiocarbon dated dataset. In this data set, once we have excluded all samples marked as "Unusable", samples missing an age and/or error estimate, samples generated using any method that is not radiocarbon dating, and samples that fall outside of the calibration age range (this should be anything < 400 years BP for marine samples, and < 200 years for terrestrial samples) as well as restricting samples to Holocene dates (0 - 12000 years BP), there are:  
* 1,529 sites in total  
* 823 sites that have 1 sample (54% of sites)  
* 299 sites that have 2 samples (20% of sites)  
* 129 sites that have 3 samples (8% of sites)  
* 97 sites that have 4 samples (6% of sites)  
* 181 sites that have 5 or more samples (12% of sites)  

So, for our 1,500 simulated sites, we should generate:  
* 50% with 1 sample only (ancient, recent, random)  
* 20% with 2 samples (bracketed)  
* 15% with 3 or 4 samples (random)  
* 15% with "full" sampling  
...
cal.emulated        <- calibrate_samples(x = emulated.data[[1]]$age, 
                                         errors = emulated.data[[1]]$error, 
                                         calCurves = 'shcal20', 
                                         normalised = normalised, 
                                         ncores = ncores)
```
